# Question
The `question` component will render a nice question for trivia purposes.
...
5. Expand to tree on the left search for `Question` → `Question {min} / {max}` adjust the current and maximum question numbers.
6. Expand to tree on the left search for `Question` → `{Question}` adjust the question text.
